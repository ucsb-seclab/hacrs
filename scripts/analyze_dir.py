...
def analyze_single(dir_path):
    if "nrfin00056" in dir_path or "NRFIN_00056" in dir_path:
        return
    print "dir_path:", dir_path
    if dir_path[-1] == "/":
        dir_path = dir_path[:-1]
    print "analyze_single", dir_path
    bin_name = os.path.dirname(os.path.dirname(dir_path)).split("/")[-1]
    res_dir = os.path.join("/results", bin_name)
    experiment_name = os.path.basename(os.path.dirname(dir_path))
    ...
    pool = Pool(8)
    traces_iter = pool.imap(get_trace, to_run, chunksize=10)
